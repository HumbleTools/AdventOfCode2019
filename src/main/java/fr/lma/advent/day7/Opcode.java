...
public enum Opcode {
	...
	/**
	 * Sums the first two parameters and stores the result at the index shown by the
	 * third parameter.
	 */
	SUM(1, 3, (operation) -> {
	...
	/**
	 * Multiplies the first two parameters and stores the result at the index shown
	...
	/**
	 * Gets user input and stores it at the index shown by the first parameter. At
	 * the the moment, input is faked.
	 */
	USER_INPUT(3, 1, (operation) -> {
		final Integer param0 = operation.getParams()[0];
		operation.getFullInput()[param0] = Advent_7.getOperationInput();
		return new OperationResult();
	}),
